...
@pytest.mark.anyio
async def test_reads_the_handle_for_an_owner_and_vendor(supabase):
    fake = supabase([{"context_id": "ctx-7"}])

    assert await get_browser_context(KWAMI_UUID, "browserbase") == "ctx-7"
...
@pytest.mark.anyio
async def test_an_owner_with_no_saved_profile_reads_as_none(supabase):
    supabase([])
    assert await get_browser_context(KWAMI_UUID, "browserbase") is None


@pytest.mark.anyio
async def test_the_vendor_is_matched_case_insensitively(supabase):
    fake = supabase([{"context_id": "ctx-7"}])

    await get_browser_context(KWAMI_UUID, "  BrowserBase ")
...
@pytest.mark.anyio
async def test_the_row_is_upserted_on_owner_and_vendor(supabase):
    """Two sessions starting at once must not leave two profiles for one user."""
    fake = supabase([{"context_id": "ctx-7"}])

    await save_browser_context(KWAMI_UUID, "browserbase", "ctx-7")
...
@pytest.mark.anyio
async def test_timestamps_are_sent_explicitly(supabase):
...
@pytest.mark.anyio
async def test_a_uuid_owner_is_linked_to_the_user_for_self_service_deletion(supabase):
    fake = supabase([{}])
...
@pytest.mark.anyio
async def test_a_telephony_identity_owner_stores_no_user_id(supabase):
    ...
    await save_browser_context(TELEPHONY_IDENTITY, "browserbase", "ctx-7")
...
async def test_unknown_vendors_are_rejected_before_the_database(supabase, vendor: str):
    """Otherwise the CHECK constraint turns a client mistake into a 500."""
    fake = supabase([])

    with pytest.raises(UnsupportedVendorError):
        await save_browser_context(KWAMI_UUID, vendor, "ctx-7")
...
async def test_a_blank_owner_is_rejected(supabase, owner: str):
    """A blank key would collide every anonymous session onto one profile."""
    supabase([])
    with pytest.raises(ValueError):
        await save_browser_context(owner, "browserbase", "ctx-7")
    with pytest.raises(ValueError):
        await get_browser_context(owner, "browserbase")


@pytest.mark.anyio
@pytest.mark.parametrize("context_id", ["", "   "])
async def test_a_blank_handle_is_rejected(supabase, context_id: str):
    supabase([])
    with pytest.raises(ValueError):
        await save_browser_context(KWAMI_UUID, "browserbase", context_id)


@pytest.mark.anyio
async def test_oversized_values_are_rejected(supabase):
    supabase([])
    with pytest.raises(ValueError):
        await save_browser_context(KWAMI_UUID, "browserbase", "c" * 300)
    with pytest.raises(ValueError):
        await save_browser_context("o" * 300, "browserbase", "ctx-7")
...
@pytest.mark.anyio
async def test_deleting_without_a_vendor_clears_every_profile(supabase):
    fake = supabase([{"id": "1"}, {"id": "2"}])

    assert await delete_browser_context(KWAMI_UUID) == 2
...
@pytest.mark.anyio
async def test_deleting_with_a_vendor_clears_only_that_one(supabase):
    fake = supabase([{"id": "1"}])

    assert await delete_browser_context(KWAMI_UUID, "browser_use") == 1
