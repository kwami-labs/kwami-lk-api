...
class TestResolveOwnedKwami:
    def test_it_returns_the_owned_row(self, fake_supabase, tenant):
        row = resolve_owned_kwami(tenant.user_id, tenant.kwami_id)
        assert row["id"] == tenant.kwami_id
        assert row["user_id"] == tenant.user_id

    def test_another_users_kwami_is_indistinguishable_from_a_missing_one(
        ...
        with pytest.raises(KwamiNotFoundError):
            resolve_owned_kwami(other_tenant.user_id, tenant.kwami_id)
        with pytest.raises(KwamiNotFoundError):
            resolve_owned_kwami(other_tenant.user_id, "00000000-0000-0000-0000-000000000000")

    @pytest.mark.parametrize(
        ("user_id", "kwami_id"),
        [("", "k1"), ("u1", ""), ("", ""), (None, "k1"), ("u1", None)],
    )
    def test_a_blank_id_short_circuits_without_a_query(
        ...
        with pytest.raises(KwamiNotFoundError):
            resolve_owned_kwami(user_id, kwami_id)

    def test_the_default_column_set_carries_the_config_blob(self, fake_supabase, tenant):
        assert "config" in KWAMI_COLUMNS
        assert "config" in resolve_owned_kwami(tenant.user_id, tenant.kwami_id)

    def test_the_column_set_is_passed_through_to_select(self, monkeypatch, fake_supabase, tenant):
        ...
        resolve_owned_kwami(tenant.user_id, tenant.kwami_id, columns="id")
        assert seen == ["id"]


class TestResolveKwami:
    def test_it_finds_a_kwami_without_a_user_filter(self, fake_supabase, tenant):
        """For /internal/* routes, where there is no end user to scope by."""
        assert resolve_kwami(tenant.kwami_id)["id"] == tenant.kwami_id

    def test_a_missing_kwami_raises(self, fake_supabase):
        with pytest.raises(KwamiNotFoundError):
            resolve_kwami("00000000-0000-0000-0000-000000000000")

    def test_a_blank_id_short_circuits(self, monkeypatch, fake_supabase):
        def explode():
            raise AssertionError("must not reach the database")
    ...
    def test_the_column_set_is_passed_through_to_select(self, monkeypatch, fake_supabase, tenant):
        ...
        resolve_kwami(tenant.kwami_id, columns="id, name")
        assert seen == ["id, name"]
...
class TestClaimEvent:
    def test_a_first_delivery_is_claimed_and_recorded(self, fake_supabase):
        assert claim_event("stripe", "evt_1", "checkout.session.completed", {"a": 1}) is True
    ...
    def test_a_redelivery_is_refused(self, fake_supabase):
        assert claim_event("stripe", "evt_1", "t") is True
        assert claim_event("stripe", "evt_1", "t") is False
        assert len(fake_supabase.db.rows("payment_events")) == 1

    def test_the_same_id_from_a_different_provider_is_a_different_event(self, fake_supabase):
        assert claim_event("stripe", "evt_1", "t") is True
        assert claim_event("twilio", "evt_1", "t") is True

    def test_a_missing_payload_is_stored_as_an_empty_object(self, fake_supabase):
        claim_event("stripe", "evt_1", "t")
        assert fake_supabase.db.rows("payment_events")[0]["payload"] == {}

    def test_an_event_with_no_id_is_processed_but_warned_about(self, fake_supabase, caplog):
        ...
        with caplog.at_level("WARNING", logger="kwami-api.idempotency"):
            assert claim_event("stripe", "", "t") is True
        assert "carried no event id" in caplog.text
        assert fake_supabase.db.rows("payment_events") == []

    def test_a_non_unique_failure_propagates(self, monkeypatch, fake_supabase):
...
class TestCompleteEvent:
    def test_it_records_the_outcome(self, fake_supabase):
        claim_event("stripe", "evt_1", "t")
        complete_event("stripe", "evt_1", status="processed", result={"credited": 100})
    ...
    def test_it_records_a_failure(self, fake_supabase):
        claim_event("stripe", "evt_1", "t")
        complete_event("stripe", "evt_1", status="failed", error="boom")
        row = fake_supabase.db.rows("payment_events")[0]
        assert row["status"] == "failed"
        assert row["error"] == "boom"
        assert row["result"] == {}

    def test_it_is_a_no_op_without_an_event_id(self, monkeypatch, fake_supabase):
        def explode():
            raise AssertionError("must not reach the database")

        monkeypatch.setattr(idempotency, "get_supabase_admin", explode)
        complete_event("stripe", "")
...
class TestAlreadyInLedger:
    def test_it_finds_an_existing_row(self, fake_supabase, tenant):
        ...
        assert already_in_ledger("stripe:session:cs_1") is True

    def test_it_is_false_for_an_unused_key(self, fake_supabase):
        assert already_in_ledger("stripe:session:nope") is False

    def test_a_blank_key_short_circuits(self, monkeypatch, fake_supabase):
        def explode():
            raise AssertionError("must not reach the database")

        monkeypatch.setattr(idempotency, "get_supabase_admin", explode)
        assert already_in_ledger("") is False
...
def test_a_unique_violation_is_recognised_as_a_duplicate(monkeypatch, fake_supabase):
    err = APIError(
